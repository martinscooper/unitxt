...
from .eval_assist_templates import direct_assessment_template_dict, pairwise_comparison_template_dict
...
class EvalAssistLLMAsJudgeDirect(BulkInstanceMetric):
    ...
    def prepare(self):
        ...
        self.assessment_template = direct_assessment_template_dict[self.model_family]["assessment"]
        self.summarization_template = direct_assessment_template_dict[self.model_family]["summarization"]
        self.option_selection_template = direct_assessment_template_dict[self.model_family]["answer"]
        ...
        self.assessment_task_prometheus = Task(
            input_fields={
        ...
        self.option_selection_task_prometheus = Task(
            input_fields={"assessment_prompt": str, "assessment": str, "options": list},
            reference_fields={},
            prediction_type=str,
            metrics=[],
        )
    ...
    def compute(
        ...
        assessment_task = (
            ...
            else self.assessment_task_prometheus
        )
        summarization_task = self.summarization_task
        option_selection_task = (
            self.option_selection_task
            if self.evaluator_name != EvaluatorNameEnum.PROMETHEUS
            else self.option_selection_task_prometheus
        )
        ...
        if self.evaluator_name == EvaluatorNameEnum.PROMETHEUS:
            ...
            assessment_outputs = [
                output.split("[RESULT]")[0].strip() for output in assessment_outputs
            ]
            ...
            selection_instances = [
                ...
                for assessment_prompt, assessment_output, criteria_option_names in zip(
                    assessment_prompts, assessment_outputs, criteria_option_names_list
                )
            ]
            ...
            selection_instances = [
                ...
                for assessment_prompt, assessment_output, criteria_description, score_option_instruction, criteria_option_names in zip(
                    assessment_prompts,
                    assessment_outputs,
                    criteria_description_list,
                    score_option_instruction_list,
        ...
        if self.option_selection_strategy == OptionSelectionStrategyEnum.PARSE_OUTPUT_TEXT or parse_output_logprobs_failed:
            ...
            selections = [
                self._parse_completion(text_completion, criteria_option_names)
                for text_completion, criteria_option_names in zip(
                    option_selection_outputs, criteria_option_names_list
                )
            ]
